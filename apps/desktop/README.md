...
| `AZURE_CLIENT_ID` / `AZURE_CLIENT_SECRET` | Windows code signing — service principal auth for CI (Trusted Signing has no interactive login) | Azure Portal → Microsoft Entra ID → App registrations → new registration, then assign it the "Trusted Signing Certificate Profile Signer" role on the Trusted Signing account, then Certificates & secrets → new client secret | ⏳ pending — CEO created the App Registration but Azure's role-assignment picker isn't listing it as a selectable member; see NEW-43 comments for the propagation-delay / directory-permission / Azure CLI workaround options |
