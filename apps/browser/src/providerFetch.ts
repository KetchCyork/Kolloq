 ...
 * Tauri webview is rejected by CORS before it leaves the process (the same wall this codebase
 * already works around for the OAuth token endpoint via Rust). Without a native relay the AI SDK's
 * request fails and — because a stream error surfaces as an empty completion rather than a thrown
 * error — the user just sees a blank reply. That's the "none of the agents responded" bug.
 *
