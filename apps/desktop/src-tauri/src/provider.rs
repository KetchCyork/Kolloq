...
/// codebase already works around for the OAuth token endpoint (see `oauth.rs` and
/// `subscriptionAuth.ts`). Making the same request from Rust isn't subject to that browser-only
/// restriction, so agents can actually get a response.
///
