...
| `AZURE_CLIENT_ID` / `AZURE_CLIENT_SECRET` | Windows code signing — service principal auth for CI (Trusted Signing has no interactive login) | Azure Portal → Microsoft Entra ID → App registrations → new registration, then assign it the "Trusted Signing Certificate Profile Signer" role on the Trusted Signing account, then Certificates & secrets → new client secret | ❌ still wrong (2026-07-26) — the 2026-07-26 15:02 UTC Windows release run (with the corrected tenant ID) now fails one step later, at `az login --service-principal`, with `AADSTS700016: Application with identifier '***' was not found in the directory '***'`. This means the app registration behind `AZURE_CLIENT_ID` does not exist as a service principal in the tenant named by `AZURE_TENANT_ID` — almost always because the App Registration was created under a *different* Entra ID directory/tenant than the one now in `AZURE_TENANT_ID` (e.g. a personal/default tenant vs. the org tenant that owns the Trusted Signing account), or `AZURE_CLIENT_ID` itself is the wrong GUID (Object ID instead of Application (client) ID). To fix: open the exact App Registration in Azure Portal, confirm its **Overview → "Directory (tenant) ID"** field is byte-for-byte identical to the `AZURE_TENANT_ID` secret, and re-copy its **Overview → "Application (client) ID"** into `AZURE_CLIENT_ID`. If the app's tenant ID doesn't match, either recreate the App Registration in the correct tenant or update `AZURE_TENANT_ID` to the app's actual tenant (whichever also hosts the Trusted Signing account and its role assignment). Re-set with `gh secret set AZURE_CLIENT_ID --repo KetchCyork/Open-Work` after confirming. |
