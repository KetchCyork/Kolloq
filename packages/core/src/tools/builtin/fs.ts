...
/** Resolves `requested` against `rootDir` and rejects any path that escapes it (`..`, symlink tricks, absolute overrides). */
function resolveSandboxed(rootDir: string, requested: string): string {
  ...
  return [
    defineTool({
      ...
      execute: async ({ path: requested }) => {
        const target = resolveSandboxed(rootDir, requested);
