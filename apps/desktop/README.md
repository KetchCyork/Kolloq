...
| `AZURE_CLIENT_SECRET` | Windows code signing — service principal password for the app above | Azure Portal → the App Registration → Certificates & secrets → new client secret → copy the **Value** column immediately (shown once, at creation time only) | ❌ still wrong after a second attempt (2026-07-26) — the 2026-07-26 15:50 UTC run failed with `AADSTS7000215`; the secret was updated (per founder comment at 17:22 UTC) and the 2026-07-26 17:22 UTC re-run hit the **identical** `AADSTS7000215: Invalid client secret provided... not the client secret ID` error at 17:38 UTC, byte-for-byte the same failure. This means the value copied the second time was *still* the Secret ID, not the Secret Value — a very easy mix-up because Azure's Certificates & secrets list shows the Secret ID column permanently but only shows the Value column once, immediately after creation, then replaces it with `***` forever. **Self-check before saving:** a Secret ID is always GUID-shaped (`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`, 36 chars, 4 dashes). A Secret Value is a shorter opaque string with no fixed dash pattern (Azure's newer secret format looks like `AbC8Q~xyz123...`). If what's about to be pasted into `gh secret set` matches the GUID pattern, it is the wrong field — stop and re-open Certificates & secrets. To fix for real: create a **brand-new client secret** (old ones can be deleted after), and the instant Azure shows the "New client secret" panel, copy only the **Value** field (the row *without* a column header reading "Secret ID"), then immediately run `gh secret set AZURE_CLIENT_SECRET --repo KetchCyork/Open-Work` with that value before navigating away — it is never shown again after the tab is left. |
